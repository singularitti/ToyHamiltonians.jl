...
!!! info
    The style guide is always a work in progress, and not all ToyHamiltonians code
    follows the rules. When modifying ToyHamiltonians, please fix the style violations
...
ToyHamiltonians uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/ToyHamiltonians.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the ToyHamiltonians directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to ToyHamiltonians have
